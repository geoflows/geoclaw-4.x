        ...
        if data_files is None:
            ...
            if path not in owners:
                ...
                try:
                    ...
                    print ("I/O error(%s): %s" % (err.errno, err.strerror))
                    raise
                except:
                    raise
...
def make_amrclawdatafile(clawdata):
    ...
    if len(clawdata.inratx) < max(abs(clawdata.mxnest)-1, 1):
        raise ValueError("*** Error in data parameter: " + "require len(inratx) >= %s " % max(abs(clawdata.mxnest) - 1, 1))
    ...
    if clawdata.mxnest < 0:
        ...
        if len(clawdata.inraty) < max(abs(clawdata.mxnest)-1, 1):
            raise ValueError("*** Error in data parameter: " + "require len(inraty) >= %s " % max(abs(clawdata.mxnest) - 1, 1))
        ...
        if len(clawdata.inratt) < max(abs(clawdata.mxnest)-1, 1):
            raise ValueError("*** Error in data parameter: " + "require len(inratt) >= %s " % max(abs(clawdata.mxnest) - 1, 1))
...
class DigclawInputData(Data):
    ...
    def __init__(self, ndim):
        ...
        self.add_attribute('kappita',   0.0001,  'characteristic grain diameter parameter in kperm (m)')
